...
> §1이 응답 *모양*이면, 여기는 커맨드(생성/수정/상태변경)가 *무엇을 어디서* 반환하는지.

- **리소스 표현을 반환한다** (201/200). 클라가 "수정완료 → 상태 교체"할 때 **서버 생성/계산값(id·createdAt·updatedAt 등)까지 권위있게** 받게 하기 위함. (id만 반환/204는 프론트가 서버값을 모르게 만들어 재조회를 강요 — 지양.)
  - 예외: 클라가 화면에 쓰지 않는 **순수 액션**(로그아웃·논리삭제 등)만 `204`.
- **데이터 출처 = 애그리거트 직접 투영 (재조회 금지).** 커맨드 후 QueryService로 다시 읽지 않는다 (DB 풀 오버헤드·경쟁 회피, CQRS상 커맨드가 쿼리측 비의존).
  - 애그리거트가 **`HasPrimitives<P>`**(`@lib/domain`)를 **옵트인** 구현 → `toPrimitives()`로 **민감필드(password 등) 제외**한 원시 표현 반환. (base에 강제하지 않음 — `domain.md` 참조)
  - UseCase 반환 타입 = 그 `XxxPrimitives` (도메인 소유). 매핑은 `toPrimitives()` **한 곳**에 집중.
- **presentation은 커맨드 전용 변환 메서드로 매핑.** 트랜스포머에 `fromPrimitives(XxxPrimitives)`를 두고, **쿼리측 `toDetailResponse(ReadModel)`과 분리**한다 (출처가 다르면 한쪽 변경이 다른 쪽으로 파급되지 않게 — 구조가 같아도 별도 메서드. `conventions.md §3`·"격리 위한 가치 있는 중복").
- **⭐ 커맨드 응답 DTO ≠ 쿼리 응답 DTO (완전 분리, 공유·상속 금지).** 메서드뿐 아니라 **반환 타입(클래스)까지 분리**한다 — 커맨드는 전용 **`XxxCommandResponseDto`**(출처 = `toPrimitives()`), 쿼리 상세는 **`XxxDetailResponseDto`**(출처 = ReadModel). **모양이 같아도 같은 클래스를 쓰거나 공통 Base를 상속하지 않는다.**
  - 근거: ① 두 출처(애그리거트 투영 vs ReadModel)는 **능력이 다르다** — 쿼리는 JOIN/계산 필드를 붙일 수 있고 커맨드 투영은 못 한다. 요구사항이 바뀌어 한쪽이 발산하면, 공유 타입·공유 Base는 **변경을 반대편으로 파급**시킨다. ② CQRS상 **Command 자원과 Read Model 자원은 1:1이 아니다**(no one-to-one). → 출처가 다른 응답은 처음부터 독립 클래스로 격리(빈 중복이라도 **격리가 만드는 가치 있는 중복**).
  - 네이밍: 커맨드 `{Entity}CommandResponseDto`(`{entity}-command.response.dto.ts`) / 쿼리 상세 `{Entity}DetailResponseDto`. 커맨드는 출처가 단일 `XxxPrimitives`라 **register/update/상태변경이 한 커맨드 DTO를 공유**해도 됨(출처가 같음). 쿼리와만 분리.
  - ⚠️ §3의 "공통 코어 Base 상속"은 **역할 간** 격리용이며, **커맨드↔쿼리 사이엔 Base 공유도 금지**(그 Base가 재결합 지점).
- **서버 생성분만 의미 있다**: 클라가 보낸 입력을 그대로 echo할 필요는 없으나, 리소스 표현 반환이 프론트 상태교체 DX엔 유리해 표현 전체를 준다.
