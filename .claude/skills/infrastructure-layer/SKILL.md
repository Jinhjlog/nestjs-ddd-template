...
- `patterns/domain-service-impl.md`: Domain Service 구현체 (LookupService) 패턴
...
│   └── index.ts
├── adapters/                            # Port 구현체 (optional — application/ports/* 대응)
├── ohs/                                 # OHS 구현체 (optional — application/ohs/* 대응, BC 간 공개 API)
└── schedulers/                          # 스케줄러·배치 작업 (optional)
```

> `adapters`/`ohs`/`schedulers`는 **필요할 때만** 생성(대부분 모듈은 `repositories`/`services`/`mappers`만). 통합 패턴 선택은 `rules/domain.md` "통합 패턴" 표 참조. 실증 레퍼런스: `file-upload` 모듈.
...
5. **Domain Service 구현체 생성** (필요시) → LookupService 패턴 구현
