...
**사용 시점**: **다른 바운디드 컨텍스트의 데이터를 우리 DB로 조회/번역**(LookupService)하거나, 도메인 규칙을 캡슐화할 때.

> ⚠️ **외부 시스템/기술(결제 게이트웨이·스토리지·OAuth·메시징·이메일 등)은 Domain Service가 아니라 Port다** (`application/ports/` → `infra/adapters/`, `rules/domain.md` 통합 패턴 표). 판별: **"다른 기술로 교체 가능한가?" Yes → Port.** Domain Service는 *우리 DB로 타 BC를 읽는 LookupService* 또는 *순수 도메인 규칙*에만 쓴다. (결제 *게이트웨이 호출*은 Port, 결제 *데이터를 우리 DB에서 읽기*는 LookupService — 외부 통신이면 Port다.)
...
| 조건                                   | 패턴                        | 예시                       |
| -------------------------------------- | --------------------------- | -------------------------- |
| 타 BC 데이터를 우리 DB로 조회/번역     | `abstract class`            | CategoryLookupService      |
| 외부 시스템/기술 (결제·스토리지·OAuth) | **Port** (`application/ports/`) | **Domain Service 아님** — `domain.md`·`file-upload` 참조 |
...
- 여러 엔티티를 조합하는 복잡한 로직
- 다른 바운디드 컨텍스트의 데이터가 필요한 경우 (LookupService)
- 비즈니스 규칙 검증이 필요한 경우
- 도메인 지식이 필요한 계산
- 단일 엔티티로 해결할 수 없는 로직
